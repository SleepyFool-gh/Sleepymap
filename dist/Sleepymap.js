...
Macro.add(['place_mapview'], {
    handler() {
        const name = this.name;
        const argObj = new ArgObj(name, MAPVIEW_TEMPLATE, this.args);
        ArgObj.add_metadata('name', name, argObj);
        const mapview = new Mapview(argObj);
        mapview.$mapview.appendTo(this.output);
    }
});
// returns map object
class Mapview {
    ...
    new_aperture() {
        ...
        const { maparray, columns, grid_travel, position } = this_map;
        const rows = maparray.length / columns;
        const aperture = {};
        if (grid_travel) {
            const aperture_x = this.x_span ?? columns;
            const aperture_y = this.y_span ?? rows;
    ...
    update() {
        // fetch map data
        const this_map = maps[this.mapname];
        const { position } = this_map;
        // check aperture
        const aperture = this.new_aperture();
        if (
            (this.aperture.min_x !== aperture.min_x)    ||
            (this.aperture.min_y !== aperture.min_y)    ||
        ...
        if (
            ...
            (this.position.mapnode !== position.mapnode)
        ) {
    ...
    #is_traversable(i) {
        const this_map = maps[this.mapname];
        const { grid_travel, maparray, position, exits, mapnodes } = this_map;
        const id = maparray[i];
        ...
        if (grid_travel) {
            return Object.values(exits.grid[i]).some( dir => dir.has(i) );
        }
        else {
    ...
    #create_tile(i) {
        // fetch map data
        const this_map = maps[this.mapname];
        const { maparray, mapnodes, position, grid_travel, columns, entities } = this_map;
        ...
        const traversable = this.enabled && this.#is_traversable(i);
        const dir = this.i2dir[i];
        const label = this.show_labels && grid_travel && traversable
            ? `<span class='macro-Sleepymap-label'>${options.labels[dir]}</span>`
            : '';
        const content = (mapnode.tile ?? '') + label;
        const current_position = grid_travel
        ...
        const $tile = $(document.createElement('div'))
            ...
            .attr('data-current-position'   , current_position)
            .data('current-position'        , current_position)
            .attr('data-dir'                , dir)
            .data('dir'                     , dir)
            // tile specific data
            .attr('data-mapnode-name'   , mapnode.name)
            .data('mapnode-name'        , mapnode.name)
            ...
            // mapstates
            .attr('data-disabled'   , mapnode.disabled || ! this.enabled)
            .data('disabled'        , mapnode.disabled || ! this.enabled)
            .attr('disabled'        , mapnode.disabled || ! this.enabled)
            .attr('data-hidden'     , mapnode.hidden)
            .data('hidden'          , mapnode.hidden)
            .css({ opacity: mapnode.hidden 
                ? '0' 
                : '' 
            })
            .attr('data-blocked'    , mapnode.blocked)
            .data('blocked'         , mapnode.blocked)
            .attr('data-walled'     , mapnode.walled)
            .data('walled'          , mapnode.walled)
            // wiki
            .wiki(content);

        if (this.i2entity[i] !== undefined) {
            for (const entityname of this.i2entity[i]) {
                const entity = entities[entityname];
...
$(document).on('Sleepymap:mapmove_resolved Sleepymap:map_edited', function(ev, data) {
    ...
    for (const id in mapviews) {
        const mapview = mapviews[id];
        if (mapview.autoupdate) mapview.update();
    }
});
...
function create_controller(argObj) {
    ...
    for (const key in keys) {
        ...
        // ERROR: not a valid dir
            ...
            throw new Error(`${name} — Sleepymap "${mapname}" — keyup key "${target?.dir}" is not a valid adjacent direction!`);
        }
        // ERROR: non-extant mapnode
        else if (
    ...
    $(document).on(`keyup.${namespace}`, function(ev) {
        ...
        // don't need to check anything for teleports
        if (target.dir === undefined) {
                found = true;
                Object.assign(current, target);
        }
        // grid travel, find first match
        else if (grid_travel) {
            ...
            // will run nothing if no exits because empty object
        ...
        // node travel
            ...
            const mapnode_exits = exits.node[position.mapnode];
            
            // for every single exit on object
            // find first to match all
            // will run nothing if no exits because empty object
            check_exits: for (const dir in mapnode_exits) {
                current.dir = dir;

                const exit_set = mapnode_exits[dir];
                for (const exit of exit_set) {

                    current.mapnode = exit;

                    // fetch all xys for mapnode, check them all to see if xy is valid
                    const xys = get_xys({ mapname, mapnode: exit });
                    for (const xy of xys) {
                        current.x = xy.x;
                        current.y = xy.y;
        ...
        // only run begin_mapmove if found a valid target
        if (found) {
...
function begin_mapmove(argObj) {
    ...
    // ERROR: mismatched xy & mapnode
    if (
        argObj.target_x !== undefined && 
        argObj.target_y !== undefined && 
        argObj.target_mapnode !== undefined
    ) {
        const xy = { x: argObj.target_x, y: argObj.target_y };
        const mapnode = maparray[xy2i({ xy, columns})];
        if (argObj.target_mapnode !== mapnode) {
            throw new Error(`${name} — Sleepymap "${mapname}" — input mapnode "${mapnode}" doesn't match mapnode at coordinates x: "${argObj.target_x}", y: "${argObj.target_y}"!`);
        }
    }

    const target_mapnode  = argObj.target_mapnode ?? maparray[xy2i({ xy: { x: argObj.target_x, y: argObj.target_y }, columns })] ;
    ...
    const origins = {
        ...
        xys         : grid_travel
                        ...
                        : get_xys({ mapname, mapnode: origin_mapnode }),
        entities    : [],
    };
...
// aux function to get offsets in each dir from a mapindex
