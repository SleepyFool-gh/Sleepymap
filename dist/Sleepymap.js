...
function create_mapview(argObj) {
    ...
    const aperture_stops_pathing    = argObj.aperture_stops_pathing ?? options.default.aperture_stops_pathing;
...
function find_path(argObj) {
    ...
    const disabled_stops_pathing    = argObj.disabled_stops_pathing ?? options.default.disabled_stops_pathing;
    const hidden_stops_pathing      = argObj.hidden_stops_pathing   ?? options.default.hidden_stops_pathing;
    const blocked_stops_pathing     = argObj.blocked_stops_pathing  ?? options.default.blocked_stops_pathing;
    const aperture_stops_pathing    = argObj.aperture_stops_pathing ?? options.default.aperture_stops_pathing;
    const this_map = maps[mapname];
    ...
    // ERROR: no aperture provided
    else if (aperture_stops_pathing && argObj.aperture === undefined) {
        throw new Error(`${name} — Sleepymap "${mapname}" — aperture_stops_pathing is true but no aperture provided!`);
    }
 
    const { grid_travel, exits, maparray, columns, mapnodes } = this_map;
    const rows = maparray / columns;
    const aperture = argObj.aperture ?? { x_span: columns, y_span: rows };
    ...
    if (
        ...
        throw new Error(`${name} — Sleepymap "${mapname}" — invalid min/max x/y!`);
    }
    ...
    bfs: while (head < queue.length) {
        ...
        const current_xy = i2xy({ i: current_i, columns });
        // prevent pathing through stopping mapstates
        const current_mapnode = mapnodes[maparray[current_i]];
        if (
            (disabled_stops_pathing && current_mapnode.disabled) ||
            (hidden_stops_pathing   && current_mapnode.hidden)   ||
            (blocked_stops_pathing  && current_mapnode.blocked)  ||
            (aperture_stops_pathing && (
                current_xy.x < aperture.min_x   ||
                current_xy.x > aperture.max_x   ||
                current_xy.y < aperture.min_y   ||
                current_xy.y > aperture.max_y
            ))
        ) continue;
